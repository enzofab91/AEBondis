    ...
    public void setup(final EvolutionState state, final Parameter base)
        ...
        try{
            ...
            for (line=br.readLine(); line!=null; line=br.readLine()){
                ...
                limites_barrios[Integer.parseInt(line_tokens[0])-1]=new int [line_tokens.length-1];
                for (int j=0;j<line_tokens.length-1;j++)
                    limites_barrios[Integer.parseInt(line_tokens[0])-1][j]=Integer.parseInt(line_tokens[j+1]);
            }
            br.close();
        }catch(IOException e){
